...
// Icons are now combined for easy access.
const Icons = { ...FaIcons, ...SiIcons };

// Updated and refined animation variants.
const fadeIn = {
...
const Header = ({ menuOpen, setMenuOpen }) => (
  <header className="bg-transparent text-white fixed top-0 left-0 right-0 z-50 w-full py-4 transition-all duration-300 backdrop-blur-md bg-opacity-70">
    <nav className="flex justify-between items-center px-6 sm:px-12 max-w-7xl mx-auto">
      ...
      >
        <a href="/" className="hover:text-purple-300 transition-colors tracking-wide">
          Shu Yi
        </a>
      </motion.div>
...
// Extracted separate components for modularity and readability.
  ...
  <section id="about" className="pt-32 pb-20">
    ...
    >
      <motion.div className="flex-1 text-center md:text-left" variants={fadeIn}>
        <motion.h1
          className="text-4xl sm:text-6xl font-bold mb-6 leading-tight font-['Poppins']"
          variants={fadeIn}
        >
          Hi, I'm <span className="text-purple-300">Shu Yi</span>
        </motion.h1>
        <motion.p
          className="text-xl sm:text-2xl text-purple-200 mb-8 font-light"
          variants={fadeIn}
        ...
        <motion.p
          className="text-lg leading-relaxed text-gray-300 max-w-2xl mx-auto md:mx-0"
          variants={fadeIn}
        ...
        >
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-block bg-purple-700 hover:bg-purple-800 text-white font-semibold px-6 py-3 rounded-lg transition-colors duration-300 shadow-lg"
          >
            View My Resume<Icons.FaDownload className="inline ml-2 text-sm" />
          </a>
        </motion.div>
      </motion.div>
      ...
      >
        <div className="relative w-64 h-64 sm:w-80 sm:h-80">
          ...
          <div className="relative w-full h-full bg-gradient-to-br from-purple-900 to-[#1E3163] rounded-full flex items-center justify-center overflow-hidden border-2 border-purple-500/30 shadow-2xl">
            {/* Reverted back to original text style */}
  ...
  <section id="education" className="py-20">
    <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
      ...
      <div className="space-y-8 relative">
        <div className="absolute left-4 sm:left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-purple-500/20 via-purple-500 to-purple-500/20"></div>
        {education && education.length > 0 ? (
          education.map((exp, index) => (
            ...
            >
              {/* Blinking/Pulse Effect for the circle */}
              <div className="absolute left-0 top-2 w-6 h-6 rounded-full bg-purple-500 border-4 border-[#1A1A40] animate-pulse"></div>
              <motion.div
              ...
              >
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                  ...
                  <span className="text-sm bg-purple-900/50 text-purple-300 px-3 py-1 rounded-full max-w-fit font-medium">
                    {exp.year}
                  </span>
                </div>
...
// Main component, now cleaner with sections broken out.
export default function Main() {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-[#0F0A2A] to-[#1A234D] text-white px-4">
        <motion.div
  ...
  return (
    <div className="bg-gradient-to-br from-[#0F0A2A] to-[#1A234D] text-white min-h-screen font-sans">
      ...
      {activeDemo && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 sm:p-8">
          <div className="bg-[#1a1a1a] rounded-lg shadow-xl overflow-hidden max-w-4xl lg:max-w-6xl w-full relative">
            <button
